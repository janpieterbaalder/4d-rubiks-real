/* ============================================================================
   sketch.ino — WOKWI ESP32 LOGIC-TEST variant (gamepad nagebootst met losse onderdelen).
   ----------------------------------------------------------------------------
   This is the *Wokwi ESP32* build. Wokwi cannot simulate Bluetooth, so it cannot run the
   real wireless firmware (../esp32_bluepad32/esp32_bluepad32.ino, which pairs a BT gamepad
   via Bluepad32). This variant therefore runs the SAME engine + control logic on a
   simulated ESP32, but rebuilds the controller from parts Wokwi *can* simulate: two analog
   sticks + a D-pad + four face buttons + SELECT/START. The mapping, the as-native
   navigation and the "arm a plane, give a direction" rotation model are IDENTICAL to
   ../esp32_bluepad32/ and to the 3D bench (hardware.html); only the Bluetooth link is faked.
   So colours, addressing AND the controls match the real ESP32 rig 1:1 — everything except
   the literal BT pairing, which is the hardware step.
   ...
   ---- WIRING (see diagram.json) — ESP32 DevKit v1 GPIOs ---------------------
     ...
     Power       : separate 5V supply -> LEDs (+ a 1000uF cap and common GND with the ESP32)
...
#include <esp_system.h>            // esp_random() for the scramble seed
...
#define BTN_SQUARE     33       // [] vlak 0 (XY)
#define BTN_CROSS      19       // X  vlak 1 (YZ)
#define BTN_CIRCLE     4        // O  vlak 2 (XZ)
#define BTN_TRIANGLE   16       // /\ grip (ribbe 180 / hoek +-120)   (RX2)
#define BTN_SELECT     17       // husselen                           (TX2)
#define BTN_START      18       // reset
...
void setup() {
  Serial.begin(115200);
  analogReadResolution(10);              // ESP32 ADC -> 0..1023 like the AVR rig (centre ~512)
  ...
  randomSeed(esp_random());              // ESP32 hardware RNG (no analog A8 pin here)
  ...
  Wire.begin(21, 22);                     // ESP32 default I2C (SDA 21, SCL 22)
  ...
  Serial.print(F("Vrij heap na init (byte): ")); Serial.println(ESP.getFreeHeap());   // ESP32 ~520KB; ruim zat
